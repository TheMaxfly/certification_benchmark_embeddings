...
def ollama_generate(model: str, system: str, prompt: str, options: dict) -> str:
    ...
    We prefer /api/chat (more robust for many instruct models, notably Phi),
    and fall back to /api/generate if /api/chat is not available.
    """
    # Prefer chat
    ...
    if r.status_code == 404:
        # Fallback for older Ollama builds or proxies
        ...
        for model in LLM_MODELS:
            ...
            for q in queries:
                ...
                if not ok_format:
                    ...
                    resp2 = ollama_generate(model, SYSTEM_PROMPT, repair, OLLAMA_OPTIONS)
                    resp = resp2
                    lines = parse_lines(resp)
                    ok_format = check_format(lines, TOP_RECOS)
