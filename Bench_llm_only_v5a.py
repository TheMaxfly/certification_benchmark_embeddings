...
def main():
    ...
    with psycopg.connect(DB_DSN) as conn:
        ...
        out_path = os.path.join(OUT_DIR, "llm_results_v4.jsonl")
        all_rows = []

        # Run per LLM
        for model in LLM_MODELS:
            print(f"\n=== LLM BENCH (V4 cite-based entity) {model} ===")
            latencies = []
            hits = []
            recalls = []
            cit_vals = []
            ...
            for q in queries:
                ...
                user_prompt = USER_PROMPT_TEMPLATE.format(query_text=q.query_text, evidences_block=evidences_block)
                ...
                if not ok_format:
                    retry = 1
                    resp2 = ollama_generate(model, SYSTEM_PROMPT, REPAIR_PROMPT + "\n\n" + resp, OLLAMA_OPTIONS)
                    resp = resp2
                    lines = parse_lines(resp)
                    ok_format = check_format(lines, TOP_RECOS)
